...
export abstract class Canvas {
  ...
  clrscr({
    bgColor, canvas, ctx, windowH, windowW, data,
  }: Canvas = this) {
...
class DefaultCanvas extends Canvas {
  ...
  constructor(public bgColor: string) {
    ...
    const order = [1, 2, 3, 4, 5].map(x => 0.618*x**1.618);
    this.curves = [1, ...order].map((order) => {
      let amp: number;
      if (order === 1) amp = windowH*0.1/order;
      else amp = windowH*0.2/order;
      const ret = (x: number) => amp*sin(order*(2*PI*x*0.4e-3 + phase));
  ...
  renderMain({
    ...
    this.curves.forEach((func, idx) => {
      ...
      if (idx % 2 !== 0) {
        const color = (func as any).color(this.step);
